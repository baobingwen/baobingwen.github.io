...
//	var data  = '';// 送信データ (POST)
//	setHttpRequestText( data, method, url, true );// Ajax (送信データ, 送信形式, 送信先, 非同期フラグ)
//	}
 ...
 function setHttpRequestText( data, method, url, async, result ) {// Ajax (XHR)
	// XMLHttpRequestオブジェクト生成
	var obj = setAjaxObject();
		...
		obj.onreadystatechange = function() {
			...
			if( obj.readyState==4 && obj.status==200 ){// readyState値は4で受信完了、status値は200でOK
				setHttpResponseText( obj, result );// コールバック関数
 ...
 // コールバック関数 (レスポンス処理)
 function setHttpResponseText( obj, result ) {// Ajax (XHR)
